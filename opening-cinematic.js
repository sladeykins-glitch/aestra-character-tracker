...
(function(){
  ...
  function styles(){
    ...
    s.textContent=`
      ...
      .aestra-opening{position:fixed;inset:0;z-index:70000;display:grid;place-items:center;overflow:hidden;pointer-events:none;visibility:hidden;background:
        radial-gradient(ellipse at 50% 88%,rgba(38,68,82,.16) 0%,rgba(11,24,32,.07) 30%,transparent 58%),
        linear-gradient(180deg,#020406 0%,#04080c 48%,#071017 74%,#050a0e 100%);
        opacity:0;transition:opacity .9s ease,visibility 0s linear .9s}
      .aestra-opening.show{opacity:1;visibility:visible;pointer-events:auto;transition:opacity .9s ease}
      .aestra-opening.closing{opacity:0;pointer-events:auto;visibility:visible;transition:opacity .75s ease}
      .aestra-opening.skipping{opacity:0;pointer-events:auto;visibility:visible;transition:opacity .18s ease}

      .aestra-water-horizon{position:absolute;left:-10%;right:-10%;top:57%;height:1px;background:linear-gradient(90deg,transparent,rgba(163,194,207,.07) 28%,rgba(205,218,220,.12) 50%,rgba(163,194,207,.07) 72%,transparent);filter:blur(.15px);opacity:.65}
      .aestra-water-glow{position:absolute;left:50%;top:57%;width:min(70vw,760px);height:30vh;transform:translateX(-50%);background:radial-gradient(ellipse at 50% 0%,rgba(171,202,215,.07),rgba(90,132,150,.022) 28%,transparent 67%);filter:blur(18px);opacity:.68;pointer-events:none}
      .aestra-water-sheen{position:absolute;left:50%;top:58%;width:min(82vw,980px);height:34vh;transform:translateX(-50%) perspective(420px) rotateX(70deg);transform-origin:top center;background:repeating-linear-gradient(180deg,rgba(179,207,218,.03) 0 1px,transparent 1px 18px);mask-image:linear-gradient(to bottom,rgba(0,0,0,.7),transparent 86%);opacity:.34;animation:aestraSheen 15s ease-in-out infinite;pointer-events:none}
      .aestra-mist{position:absolute;left:-25%;width:150%;height:24vh;border-radius:50%;filter:blur(38px);opacity:.065;pointer-events:none;background:radial-gradient(ellipse,rgba(176,198,204,.7),rgba(80,108,117,.18) 42%,transparent 70%)}
      .aestra-mist.one{top:25%;animation:aestraMistOne 22s ease-in-out infinite alternate}
      .aestra-mist.two{top:64%;opacity:.045;animation:aestraMistTwo 26s ease-in-out infinite alternate-reverse}
      .aestra-motes{position:absolute;inset:0;pointer-events:none;opacity:.1;background-image:
        radial-gradient(circle at 18% 31%,rgba(226,232,229,.75) 0 .7px,transparent .9px),
        radial-gradient(circle at 73% 23%,rgba(226,232,229,.65) 0 .6px,transparent .8px),
        radial-gradient(circle at 82% 67%,rgba(202,218,222,.6) 0 .7px,transparent .9px),
        radial-gradient(circle at 31% 75%,rgba(202,218,222,.5) 0 .65px,transparent .85px),
        radial-gradient(circle at 57% 38%,rgba(226,232,229,.45) 0 .55px,transparent .75px);
        animation:aestraMotes 20s ease-in-out infinite alternate}
      ...
      .aestra-dream-line{position:absolute;max-width:min(760px,88vw);padding:18px 22px;margin:0;color:rgba(239,240,235,.96);font:400 clamp(1.22rem,3vw,2.3rem)/1.42 Georgia,'Times New Roman',serif;letter-spacing:.012em;text-shadow:0 0 8px rgba(220,232,235,.03),0 3px 24px rgba(0,0,0,.72);opacity:0;filter:blur(6px);transform:scale(.995);transition:opacity 1.65s cubic-bezier(.22,.61,.36,1),filter 1.85s ease,transform 1.85s ease,text-shadow 1.65s ease,letter-spacing 1.65s ease}
      .aestra-dream-line.show{opacity:1;filter:blur(0);transform:scale(1);letter-spacing:.018em;text-shadow:0 0 18px rgba(193,219,228,.14),0 0 38px rgba(165,200,213,.055),0 3px 24px rgba(0,0,0,.72)}

      .aestra-ripple{position:absolute;left:50%;top:57%;width:44px;height:13px;border:1px solid rgba(181,209,220,.13);border-radius:50%;transform:translate(-50%,-50%) scale(.25);pointer-events:none;opacity:0;animation:aestraRipple 4s cubic-bezier(.12,.58,.2,1) forwards}
      .aestra-ripple.secondary{display:none}

      .aestra-awaken-wrap{position:absolute;left:50%;top:66%;transform:translate(-50%,10px);display:grid;place-items:center;gap:10px;opacity:0;transition:opacity 1.35s ease,transform 1.35s ease;pointer-events:none}
      ...
      .aestra-opening-skip{position:absolute;z-index:4;right:max(18px,env(safe-area-inset-right));top:max(16px,env(safe-area-inset-top));border:0!important;background:transparent!important;color:rgba(216,220,216,.35)!important;font-size:.56rem!important;letter-spacing:.12em!important;text-transform:uppercase!important;padding:10px!important;min-height:0!important;cursor:pointer!important}
      .aestra-opening-skip:hover,.aestra-opening-skip:focus-visible{color:rgba(235,231,216,.9)!important}
      .aestra-opening-mark{position:absolute;z-index:3;left:50%;bottom:max(22px,env(safe-area-inset-bottom));transform:translateX(-50%);font-size:.44rem;letter-spacing:.23em;text-transform:uppercase;color:rgba(159,179,185,.16);white-space:nowrap}

      @keyframes aestraRipple{0%{opacity:0;transform:translate(-50%,-50%) scale(.25)}12%{opacity:.3}58%{opacity:.1}100%{opacity:0;transform:translate(-50%,-50%) scale(18,8)}}
      @keyframes aestraSheen{0%,100%{opacity:.28;transform:translateX(-50%) perspective(420px) rotateX(70deg) translateY(0)}50%{opacity:.38;transform:translateX(-50%) perspective(420px) rotateX(70deg) translateY(4px)}}
      ...
      @keyframes aestraMotes{from{transform:translateY(0);opacity:.07}to{transform:translateY(-6px);opacity:.11}}
      ...
      @media(prefers-reduced-motion:reduce){.aestra-water-sheen,.aestra-mist,.aestra-motes{animation:none!important}.aestra-ripple{animation-duration:.8s!important}.aestra-dream-line{transition-duration:.22s!important;filter:none!important}.aestra-awaken-wrap{transition-duration:.22s!important}}
    `;
  ...
  function ensureOverlay(){
    ...
    overlay.querySelector('.aestra-opening-skip').onclick=e=>{e.preventDefault();e.stopPropagation();skipCinematic()};
  ...
  async function showLine(text,hold,token){
    ...
    line.textContent='';
    await sleep(reducedMotion?35:420);
    if(token!==runToken)return false;

    line.textContent=text;
    line.classList.add('show');
    await sleep(reducedMotion?120:1650);
    if(token!==runToken)return false;

    spawnRipple();
    await sleep(reducedMotion?300:hold);
    if(token!==runToken)return false;

    line.classList.remove('show');
    await sleep(reducedMotion?120:1750);
    if(token!==runToken)return false;

    line.textContent='';
    await sleep(reducedMotion?45:420);
  ...
  async function play({createAfter=false}={}){
    ...
    const o=ensureOverlay();
    o.classList.remove('closing','skipping');
    ...
    await sleep(reducedMotion?80:750);
  ...
  async function finalLine(token){
    ...
    if(token!==runToken)return;
    spawnRipple();
    await sleep(reducedMotion?160:1150);
    if(token!==runToken)return;
  ...
  function closeOverlay({fast=false}={}){
    ++runToken;
    const shouldCreate=currentMode==='first';
    const o=ensureOverlay();
    const active=document.activeElement;
    if(active&&o.contains(active))active.blur?.();
    o.classList.remove('closing','skipping');
    o.classList.add(fast?'skipping':'closing');
    const delay=reducedMotion?80:(fast?200:760);
    setTimeout(()=>{
      o.classList.remove('show','closing','skipping');
      o.querySelector('.aestra-awaken-wrap').classList.remove('show');
      const line=o.querySelector('.aestra-dream-line');
      line.classList.remove('show');
      line.textContent='';
  ...
  function finish(){
    closeOverlay({fast:false});
  }

  function skipCinematic(){
    closeOverlay({fast:true});
  }
