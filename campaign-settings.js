...
(function(){
  ...
  function boot(){
    ...
    if(gmTab)new MutationObserver(()=>renderGMPanel()).observe(gmTab,{attributes:true,attributeFilter:['class']});
    document.addEventListener('click',e=>{
      if(e.target.closest?.('.ubp-main-add,[data-bh-add-skill],#addClassBtn,#addSkillBtn'))setTimeout(applyPickerVisibility,0);
      if(e.target.closest?.('[data-view="gm"]'))setTimeout(renderGMPanel,0);
    },true);
    window.addEventListener('pageshow',()=>{applyPickerVisibility();renderGMPanel()},{passive:true});
  }
