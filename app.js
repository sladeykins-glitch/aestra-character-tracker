...
if(!document.getElementById('gmMpOther')){
  ...
  if(host){const label=document.createElement('label');label.textContent='Other';const input=document.createElement('input');input.id='gmMpOther';input.type='number';label.appendChild(input);host.appendChild(label)}
}
...
await loadLayer('./session-tools.js?v=1');