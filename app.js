...
async function enterApp(){
  ...
  const gmTab=document.querySelector('[data-view="gm"]');
  gmTab.classList.toggle('hidden',!isGM);
  els.connectionBadge.textContent = isGM ? `GM · ${currentUser?.email || 'Connected'}` : (currentUser?.email || 'Connected');
...
async function saveCharacter(){
  ...
  setSaveStatus('Saving…'); const payload={...state,owner_id:currentUser.id,campaign_id:CONFIG.campaignId,updated_at:new Date().toISOString()}; delete payload.id; delete payload.created_at;
  let result; if(currentCharacterId) result=await supabase.from('characters').update(payload).eq('id',currentCharacterId).select().single(); else result=await supabase.from('characters').insert(payload).select().single();
...
function gmCard(c){ const statuses=(c.statuses||[]).map(s=>`<span class="badge">${escapeHtml(s)}</span>`).join('') || '<span class="muted">No statuses</span>'; return `<article class="panel gm-card"><div class="gm-name"><div><p class="eyebrow">${escapeHtml(c.player_name||'Player')}</p><h3>${escapeHtml(c.name||'Unnamed')}</h3></div><span class="badge">Lv ${Number(c.level)||1}</span></div><div class="mini-resources"><div class="mini"><span>HP</span><strong>${Number(c.hp_current)||0}/${Number(c.hp_max)||0}</strong></div><div class="mini"><span>MP</span><strong>${Number(c.mp_current)||0}/${Number(c.mp_max)||0}</strong></div><div class="mini"><span>IP</span><strong>${Number(c.ip_current)||0}/${Number(c.ip_max)||0}</strong></div></div><div class="chips">${statuses}</div></article>`; }
function escapeHtml(v){ return String(v??'').replace(/[&<>'"]/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[ch])); }
async function loadGMDashboard(){ els.gmCards.innerHTML='<p class="muted">Loading characters…</p>'; if(isDemo){ els.gmCards.innerHTML=gmCard(state); return; } if(!isGM){ els.gmCards.innerHTML='<p class="muted">GM access required.</p>'; return; } const { data,error }=await supabase.from('characters').select('*').eq('campaign_id',CONFIG.campaignId).order('name'); if(error){ els.gmCards.innerHTML=`<p class="message">${escapeHtml(error.message)}</p>`; return; } els.gmCards.innerHTML=data?.length ? data.map(gmCard).join('') : '<p class="muted">No characters yet.</p>'; }
...
els.demoBtn.addEventListener('click',enterDemo); els.logoutBtn.addEventListener('click',async()=>{ if(supabase) await supabase.auth.signOut(); currentUser=null; currentCharacterId=null; showAuth(); }); els.saveBtn.addEventListener('click',saveCharacter); els.refreshGmBtn.addEventListener('click',loadGMDashboard); els.fpMinus.addEventListener('click',()=>{state.fabula_points=Math.max(0,state.fabula_points-1);renderResources();markDirty();}); els.fpPlus.addEventListener('click',()=>{state.fabula_points+=1;renderResources();markDirty();}); els.portraitUrl.addEventListener('change',()=>{state.portrait_url=els.portraitUrl.value.trim();renderPortrait();markDirty();}); els.portraitImg.addEventListener('error',()=>{els.portraitImg.classList.add('hidden');els.portraitFallback.classList.remove('hidden');});
document.querySelectorAll('.resource-card').forEach(card=>card.querySelectorAll('[data-delta]').forEach(btn=>btn.addEventListener('click',()=>{const short=card.dataset.resource,key=`${short}_current`,maxKey=`${short}_max`;pullForm();state[key]=clamp(state[key]+Number(btn.dataset.delta),0,state[maxKey]);renderResources();markDirty();}))); ['hpMax','mpMax','ipMax'].forEach(id=>els[id].addEventListener('change',()=>{pullForm();renderResources();markDirty();})); [...document.querySelectorAll('#sheetView input,#sheetView select,#sheetView textarea')].forEach(el=>{if(!['portraitUrl','hpMax','mpMax','ipMax'].includes(el.id)) el.addEventListener('input',markDirty);}); document.querySelectorAll('.tab').forEach(tab=>tab.addEventListener('click',async()=>{document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active'));tab.classList.add('active');const view=tab.dataset.view;els.sheetView.classList.toggle('hidden',view!=='sheet');els.gmView.classList.toggle('hidden',view!=='gm');if(view==='gm')await loadGMDashboard();}));
