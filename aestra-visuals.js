...
function installStatusLayout(){
  ...
  const home=document.getElementById('statusHome');
  if(home&&statuses.parentElement!==home) home.appendChild(statuses);
  if(oldPanel&&oldPanel!==panel){oldPanel.style.display='none'}
  decorateStatuses();
}
function decorateStatuses(){
  ...
  [...statuses.querySelectorAll('button')].forEach(btn=>{
    ...
    if(!btn.classList.contains('aestra-status')) btn.classList.add('aestra-status',`status-${info.tone}`);
    if(btn.dataset.status!==key) btn.dataset.status=key;
    if(btn.dataset.decorated!=='1'){
...
function updateAttributeEffects(){
  ...
  Object.entries(map).forEach(([attr,[baseId,currentId]])=>{
    ...
    const currentText=dieValue(currentId),baseText=dieValue(baseId);
    if(visual.dataset.current!==currentText||visual.dataset.base!==baseText){visual.innerHTML=`<span class="die-current">${currentText}</span><small>base ${baseText}</small>`;visual.dataset.current=currentText;visual.dataset.base=baseText}
    ...
    const effectText=affected[attr].length?`${affected[attr].join(' + ')} ↓`:'';
    if(effect.textContent!==effectText)effect.textContent=effectText;
    card.classList.toggle('status-affected',affected[attr].length>0);
...
function resourceEffects(){
  document.querySelectorAll('.resource-card').forEach(card=>{
    const type=card.dataset.resource;if(!card.classList.contains(`resource-${type}`))card.classList.add(`resource-${type}`);
...
function installAtmosphere(){if(document.getElementById('aestraAtmosphere'))return;const layer=document.createElement('div');layer.id='aestraAtmosphere';layer.className='aestra-atmosphere';layer.setAttribute('aria-hidden','true');layer.innerHTML='<i></i><i></i><i></i><i></i><i></i><i></i>';document.body.prepend(layer)}
function installStyles(){
  ...
  body{position:relative}.app-shell{position:relative;z-index:1}.aestra-atmosphere{position:fixed;inset:0;pointer-events:none;overflow:hidden;z-index:0;opacity:.28}.aestra-atmosphere i{position:absolute;width:3px;height:3px;border-radius:50%;background:rgba(218,180,104,.34);box-shadow:0 0 8px rgba(218,180,104,.18);animation:aestraDrift 18s linear infinite}.aestra-atmosphere i:nth-child(1){left:8%;top:92%;animation-delay:-2s}.aestra-atmosphere i:nth-child(2){left:28%;top:70%;animation-delay:-9s;animation-duration:24s}.aestra-atmosphere i:nth-child(3){left:52%;top:96%;animation-delay:-5s;animation-duration:21s}.aestra-atmosphere i:nth-child(4){left:73%;top:80%;animation-delay:-12s;animation-duration:26s}.aestra-atmosphere i:nth-child(5){left:91%;top:90%;animation-delay:-7s}.aestra-atmosphere i:nth-child(6){left:61%;top:62%;animation-delay:-15s;animation-duration:28s}@keyframes aestraDrift{0%{transform:translate3d(0,0,0);opacity:0}15%{opacity:.55}100%{transform:translate3d(18px,-75vh,0);opacity:0}}
  .combat-status-panel{margin-top:12px;border-color:rgba(196,151,73,.34);background:linear-gradient(145deg,rgba(44,34,24,.96),rgba(24,20,16,.96));box-shadow:inset 0 1px rgba(255,255,255,.025),0 10px 32px rgba(0,0,0,.18)}#statuses.chips{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.aestra-status{min-height:58px!important;display:flex!important;align-items:center!important;justify-content:flex-start!important;gap:9px!important;text-align:left!important;padding:9px 11px!important;transition:transform .16s ease,box-shadow .2s ease,border-color .2s ease,background .2s ease!important}.aestra-status:active{transform:scale(.97)}.status-icon{font-size:1.35rem;line-height:1;width:25px;text-align:center;opacity:.72}.status-copy{display:grid;gap:2px}.status-copy strong{font-size:.92rem}.status-copy small{font-size:.72rem;opacity:.64}.aestra-status.active,.aestra-status[aria-pressed="true"]{border-color:rgba(231,184,99,.75)!important;background:linear-gradient(145deg,rgba(112,75,36,.5),rgba(46,33,23,.85))!important;box-shadow:0 0 0 1px rgba(218,164,76,.15),0 0 20px rgba(218,164,76,.12)!important}.status-legend{display:flex;justify-content:space-between;gap:10px;margin-top:9px;font-size:.72rem;opacity:.58}
  .attr-card{position:relative;overflow:hidden;transition:border-color .25s ease,box-shadow .25s ease,transform .2s ease}.die-visual{display:grid;place-items:center;margin:4px 0 9px;padding:8px 4px;border:1px solid rgba(197,155,83,.18);border-radius:12px;background:radial-gradient(circle at 50% 30%,rgba(195,148,73,.1),transparent 67%)}.die-current{font-family:Georgia,serif;font-weight:700;font-size:1.55rem;line-height:1;color:#e8c786}.die-visual small{font-size:.65rem;opacity:.5;margin-top:3px}.attribute-effect{height:15px;margin-top:5px;font-size:.66rem;color:#e2aa62;text-align:center;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.attr-card.status-affected{border-color:rgba(222,164,76,.62);box-shadow:inset 0 0 20px rgba(184,126,52,.08),0 0 17px rgba(185,126,52,.08)}.attr-card.die-changed .die-current{animation:dieSettle .38s ease}@keyframes dieSettle{0%{transform:translateY(-5px) scale(1.15);opacity:.2}60%{transform:translateY(2px) scale(.97)}100%{transform:none;opacity:1}}
  .resource-hp.in-crisis{border-color:rgba(177,67,51,.62);box-shadow:inset 0 0 24px rgba(148,45,35,.08),0 0 16px rgba(148,45,35,.08)}.resource-hp.in-crisis .resource-head strong{color:#e4a091}
  ...
  @media(prefers-reduced-motion:reduce){.aestra-atmosphere{display:none}.attr-card.die-changed .die-current{animation:none}}
  `;document.head.appendChild(s)
}
function observe(){
  const statuses=document.getElementById('statuses');
  if(statuses){const mo=new MutationObserver(muts=>{if(muts.some(m=>m.type==='childList'))decorateStatuses()});mo.observe(statuses,{childList:true})}
  const sheet=document.getElementById('sheetView');
  sheet?.addEventListener('click',()=>setTimeout(()=>{updateAttributeEffects();resourceEffects()},0));
  sheet?.addEventListener('input',()=>requestAnimationFrame(()=>{updateAttributeEffects();resourceEffects()}));
  sheet?.addEventListener('change',()=>requestAnimationFrame(()=>{updateAttributeEffects();resourceEffects()}));
}
